                ...
                menu = menu.toggleable_entry(
                    ...
                    move |_window, cx| {
                        ...
                        settings::update_settings_file(fs.clone(), cx, move |settings, _cx| {
                            settings.agent.get_or_insert_default().sidebar_side = Some(position);
                        });
                    },
                );
            }
...
impl MultiWorkspace {
            ...
            move |this, _project, event, _window, cx| match event {
                project::Event::WorktreePathsChanged { old_worktree_paths } => {
                    if let Some(workspace) = workspace.upgrade() {
                        let old_key =
                            ProjectGroupKey::from_worktree_paths(old_worktree_paths, None);
    ...
    pub fn find_or_create_workspace(
        ...
        _connect_remote: impl FnOnce((), &mut Window, &mut Context<Self>) -> Task<Result<Option<()>>>
        + 'static,
